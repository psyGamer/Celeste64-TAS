...
public class Menu
{
    ...
    public readonly Stack<Menu> submenus = [];

    public string UpSound = Sfx.ui_move;
    public string DownSound = Sfx.ui_move;

    public bool IsInMainMenu = false;
    public Menu CurrentMenu => submenus.Count > 0 ? submenus.Peek().CurrentMenu : this;
    ...
    {
        if (submenus.Count > 0 && submenus.TryPeek(out var result))
        {
            if (result.CloseOpen())
            {
                Close();
    ...
    public void Update()
    {
        if (submenus.Count > 0 && submenus.TryPeek(out var topmenu))
        {
            topmenu.Update();
            return;
        }
