...
public class ImGuiRenderer
{
    ...
    public void Update()
    {
        if (Target == null || Target.IsDisposed || Target.Width != App.WidthInPixels || Target.Height != App.HeightInPixels) {
            Target?.Dispose();
            ...
            if (spriteMaterial != null && (spriteMaterial.Shader?.Has("u_matrix") ?? false))
                spriteMaterial.Set("u_matrix", Matrix.CreateOrthographicOffCenter(0f, Target!.Width, Target.Height, 0f, -1.0f, 1.0f));
        }

        var io = ImGui.GetIO();
        io.DisplaySize = new Vec2(Target.Width, Target.Height);
